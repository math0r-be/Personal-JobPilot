...
export default function NewEmailPage() {
  ...
  const [form, setForm] = useState({ jobPostingId: '', to: '', subject: '', body: '' });
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    fetch('/api/jobs').then(r => r.json()).then(data => setJobs(Array.isArray(data) ? data : []));
  }, []);
        ...
        <form onSubmit={handleSubmit} style={{ maxWidth: 720 }}>
          <div style={{ background: 'var(--paper-warm)', border: '1px solid var(--line-soft)', borderRadius: 'var(--r-lg)', padding: 24, marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input type="text" value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} required placeholder="Candidature au poste de…" style={inputStyle} />
            </div>
            <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="submit" disabled={saving} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 40, padding: '0 20px', borderRadius: 'var(--r-md)', fontSize: 13, fontWeight: 500, background: 'var(--accent)', color: 'var(--paper-warm)', border: 'none', cursor: 'pointer', opacity: saving ? 0.5 : 1 }}>
